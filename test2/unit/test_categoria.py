...
def app():
    ...
    # ⚙️ Inicializa correctamente la extensión antes del contexto
    db.init_app(app)
...
# Crea un usuario base disponible para las pruebas
@pytest.fixture
def db_session(app):
    with app.app_context():
        cliente = Usuario(
...
def test_creacion_de_categoria(app, db_session):
    with app.app_context():
        cliente = Usuario.query.first()
        categoria = Categoria(nombre="Ropa", cliente_id=cliente.id)
...
def test_conversion_categoria_a_diccionario(app, db_session):
    with app.app_context():
        cliente = Usuario.query.first()
        categoria = Categoria(nombre="Juguetes", cliente_id=cliente.id)
        db_session.add(categoria)
        db_session.commit()
...
def test_nombre_categoria_unico(app, db_session):
    with app.app_context():
        cliente = Usuario.query.first()
        categoria1 = Categoria(nombre="Videojuegos", cliente_id=cliente.id)
        db_session.add(categoria1)
        db_session.commit()

        categoria2 = Categoria(nombre="Videojuegos", cliente_id=cliente.id)
